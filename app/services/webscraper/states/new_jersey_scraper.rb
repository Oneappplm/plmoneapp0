...
module Webscraper
  module States
    class NewJerseyScraper
      SEARCH_URL = "https://newjersey.mylicense.com/verification/Search.aspx?facility=N".freeze
      ...
      def crawl!
        ...
        input = fast_wait.until { crawler.find_element(:id, "t_web_lookup__license_no") }
        input.clear
        ...
        # --- Find correct details link ---
        puts "➡️ Looking for results link..."
        details_link = wait_for_results_name_link

        if details_link
          puts "➡️ Clicking details link: #{details_link.text}"
          crawler.execute_script("arguments[0].click();", details_link)
        else
          puts "⚠️ No result link found. Taking screenshot anyway."
        end
      ...
      def wait_for_results_name_link
        40.times do
          begin
            links = crawler.find_elements(:xpath, "//a[contains(@id,'datagrid_results__ctl') and contains(@id,'_name')]")

            valid = links.find(&:displayed?)
      ...
      def save_screenshot
        ...
        image.combine_options do |c|
          ...
          c.fill "black"
          c.pointsize 14
          c.draw "text 140,1070 '#{human_date}'"
        end
        image.write(path)
