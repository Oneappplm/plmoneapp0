class Mhc::VerificationPlatformController < ApplicationController
  ...
  def get_data
    if params[:page_tab] == 'practitioner_info'
      ...
      # 🔥 IMPORTANT FIX
      unless @provider_personal_information.provider_attest
        @provider_personal_information.build_provider_attest
      end
    ...
    if params[:page_tab] == 'oig'
      ...
      @oig_webcrawler_logs = OigWebcrawlerLog.joins(:rva_information).where(rva_informations: { provider_personal_information_id: @provider_personal_information.id, tab: 'OIG' }).order(created_at: :desc)
    end
