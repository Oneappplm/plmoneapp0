class Mhc::ProviderLicensuresController < ApplicationController
  before_action :set_provider_licensure, only: [:update, :destroy]
  ...
  def create
    ...
    if @provider_licensure.save
      redirect_to mhc_verification_platform_path(
        page_tab: 'licensure',
        id: @provider_licensure.provider_attest_id
      ), notice: 'Provider licensure detail saved successfully.'
    else
      Rails.logger.error @provider_licensure.errors.full_messages.join(", ")
      redirect_to mhc_verification_platform_path(
        page_tab: 'licensure',
        id: params[:provider_licensure][:provider_attest_id]
      ), alert: 'Failed to save provider licensure detail.'
    end
  end
  ...
  def update
    if @provider_licensure.update(provider_licensure_params)
      redirect_to mhc_verification_platform_path(
        page_tab: 'licensure',
        id: @provider_licensure.provider_attest_id
      ), notice: 'Provider licensure detail updated successfully.'
    else
      Rails.logger.error @provider_licensure.errors.full_messages.join(", ")
      redirect_to mhc_verification_platform_path(
        page_tab: 'licensure',
  ...
  def set_provider_licensure
    licensure_id = params[:id] || params.dig(:provider_licensure, :id) || params[:provider_licensure_id]
