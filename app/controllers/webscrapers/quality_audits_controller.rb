class Webscrapers::QualityAuditsController < ApplicationController
  ...
  def generate_dea_bulk_report
    ...
    if rows.empty?
      return render json: {
        success: false,
        message: "No DEA crawler records found."
      }, status: :unprocessable_entity
    end

    csv_path = Reports::DeaBulkCsvExporter.new(rows: rows).call

    render json: {
      success: true,
      message: "DEA bulk report generated successfully.",
      total_rows: rows.size,
      download_url: download_dea_report_webscrapers_quality_audits_path(path: csv_path)
    }, status: :ok
  rescue => e
    render json: {
      success: false,
  ...
  def download_dea_report
    path = CGI.unescape(params[:path].to_s)
