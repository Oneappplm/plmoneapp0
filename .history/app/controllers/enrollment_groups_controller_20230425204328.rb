...
def new_group
  if request.post?
    ...
    if @enrollment_group.save
      redirect_to new_group_enrollment_groups_path, notice: 'Group has been successfully created.' and return
    end
  else
...
def edit_enrollment_group
  ...
  if	request.patch?
    if @enrollment_group.update(group_params)
      ...
      redirect_to new_group_enrollment_groups_path, notice: "#{group.name} has been successfully updated." and return
    end
  end
...
def delete_enrollment_group
  ...
  if user.destroy
    rredirect_to new_group_enrollment_groups_path, notice: "#{group.name} has been deleted."
  else
    redirect_to new_group_enrollment_groups_path, alert: 'Something went wrong'
  end
